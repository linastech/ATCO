...
export default class ExportForm extends React.Component {
	...
	validateProductOfInterest(value){
		return !value || value.length < 3 ? 'Please enter your product of interest' : null
	}
	submit(formData){
		//show loading icon
		this.setState({loading: true})

		Axios.post('/_form/submit/export', formData)
			.then(res => {
	...
	render(){
		return (
			<div className="wrapper">
				<Form id="complex-form" className={FORM.labeled} onSubmit={this.submit} getApi={this.setFormApi}>
					{({ formState }) => (
						<React.Fragment>
							...
							<label className={FORM.label} htmlFor="product-of-interest">Products of Interest *</label>
							{formState.errors['product-of-interest'] && <div className={FORM.errorBox}>{formState.errors['product-of-interest']}</div> }
							<Text className={FORM.input} field="product-of-interest" id="product-of-interest" validate={this.validateProductOfInterest} />
